import QuestionAndAnswers from "../types/QuestionAndAnswers";

/*
Round 1 teams 
Bruns vs Jessens

Round 2 teams 
Rosners vs Messerschmitts

Round 3 teams 
Gordons vs Loomis
*/

const questionsAndAnswers : QuestionAndAnswers[][] = [
    [
        {
            question: { text: "Name something you'd hate to have happen while you're taking a bath." },
            answers: [
                {
                    text: "Phone rings",
                    points: 40,
                    revealed: false,
                },
                {
                    text: "No hot water",
                    points: 35,
                    revealed: false,
                },
                {
                    text: "Doorbell rings",
                    points: 30,
                    revealed: false,
                },
                {
                    text: "Get electrocuted",
                    points: 25,
                    revealed: false,
                },
                {
                    text: "Power outage",
                    points: 20,
                    revealed: false,
                },
                {
                    text: "Slip/fall",
                    points: 15,
                    revealed: false,
                },
                {
                    text: "Someone comes in",
                    points: 10,
                    revealed: false,
                },
            ]
        },
        {
            question: { text: "Name a place you'd hate to spend even one night." },
            answers: [
                {
                    text: "In-laws' home",
                    points: 40,
                    revealed: false,
                },
                {
                    text: "Jail/prison",
                    points: 35,
                    revealed: false,
                },
                {
                    text: "Hospital",
                    points: 30,
                    revealed: false,
                },
                {
                    text: "Office",
                    points: 25,
                    revealed: false,
                },
                {
                    text: "On the couch",
                    points: 20,
                    revealed: false,
                },
                {
                    text: "Campground",
                    points: 15,
                    revealed: false,
                },
            ]
        },
        {
            question: { text: "Who in the bible would make the best member of the fellowship of the ring." },
            answers: [
                {
                    text: 'Jesus',
                    points: 40,
                    revealed: false,
                },
                {
                    text: 'Samson',
                    points: 35,
                    revealed: false,
                },
                {
                    text: 'David',
                    points: 30,
                    revealed: false,
                },
                {
                    text: 'Paul',
                    points: 25,
                    revealed: false,
                },
                {
                    text: 'Elijah',
                    points: 20,
                    revealed: false,
                },
                {
                    text: 'Zacchaeus',
                    points: 15,
                    revealed: false,
                },
            ]
        }
    ],
    [
        {
            question: { text: "Name something you stuff just as full as you can get it." },
            answers: [
                {
                    text: "Turkey",
                    points: 40,
                    revealed: false,
                },
                {
                    text: "Trash cans",
                    points: 35,
                    revealed: false,
                },
                {
                    text: "Christmas stocking",
                    points: 30,
                    revealed: false,
                },
                {
                    text: "Suitcase",
                    points: 25,
                    revealed: false,
                },
                {
                    text: "Pillow",
                    points: 20,
                    revealed: false,
                },
                {
                    text: "Teddy bear",
                    points: 15,
                    revealed: false,
                },
                {
                    text: "Purse",
                    points: 10,
                    revealed: false,
                },
                {
                    text: "Stomach",
                    points: 5,
                    revealed: false,
                },
            ]
        },
        {
            question: { text: "Name something people tend to fall out of." },
            answers: [
                {
                    text: 'Tree',
                    points: 40,
                    revealed: false,
                },
                {
                    text: 'Love',
                    points: 35,
                    revealed: false,
                },
                {
                    text: 'Bed',
                    points: 30,
                    revealed: false,
                },
                {
                    text: 'Chair',
                    points: 25,
                    revealed: false,
                },
                {
                    text: 'Car',
                    points: 20,
                    revealed: false,
                },
            ]
        },
        {
            question: { text: "What modern food would be at the last supper if it happened today." },
            answers: [
                {
                    text: 'Cheesy/mashed potatoes',
                    points: 40,
                    revealed: false,
                },
                {
                    text: 'Fried Chicken',
                    points: 35,
                    revealed: false,
                },
                {
                    text: 'Chick-fil-A',
                    points: 30,
                    revealed: false,
                },
                {
                    text: 'Jello',
                    points: 25,
                    revealed: false,
                },
                {
                    text: 'Chilli & cinnamon rolls',
                    points: 20,
                    revealed: false,
                },
                {
                    text: 'Potato salad',
                    points: 15,
                    revealed: false,
                },
                {
                    text: 'Baked beans',
                    points: 10,
                    revealed: false,
                },
            ]
        }
    ],
    [
        {
            question: { text: "When You Hear a Noise Coming From Your Basement, What Do You Pray That it ISN'T?" },
            answers: [
                {
                    text: 'Another Person',
                    points: 40,
                    revealed: false,
                },
                {
                    text: 'Ghost/Monster',
                    points: 35,
                    revealed: false,
                },
                {
                    text: 'Animals/Rats',
                    points: 30,
                    revealed: false,
                },
                {
                    text: 'Flood/Water Heater',
                    points: 25,
                    revealed: false,
                },
            ]
        },
        {
            question: { text: "Name Something You Might Be Glad Only Comes Once a Year." },
            answers: [
                {
                    text: 'Christmas',
                    points: 40,
                    revealed: false,
                },
                {
                    text: 'Birthdays',
                    points: 35,
                    revealed: false,
                },
                {
                    text: 'Tax Season',
                    points: 30,
                    revealed: false,
                },
                {
                    text: 'Anniversary',
                    points: 25,
                    revealed: false,
                },
            ]
        },
        {
            question: { text: "Name something in the Bible you shouldn't touch." },
            answers: [
                {
                    text: "The apple",
                    points: 40,
                    revealed: false,
                },
                {
                    text: "The ark of the covenant",
                    points: 35,
                    revealed: false,
                },
                {
                    text: "The Lord's anointed",
                    points: 30,
                    revealed: false,
                },
                {
                    text: "Another man's wife",
                    points: 25,
                    revealed: false,
                },
                {
                    text: "Dead animal",
                    points: 20,
                    revealed: false,
                },
                {
                    text: "Unclean/leper",
                    points: 15,
                    revealed: false,
                },
            ]
        }
    ]
]

export default questionsAndAnswers;